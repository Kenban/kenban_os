...
echo "Removing kenban_celery_1"
docker stop kenban_celery_1
docker rm kenban_celery_1
#docker image rm kenban/celery-kos:$DOCKER_TAG
...
docker "${DOCKER_BUILD_ARGS[@]}" \
    ...
    -t "kenban/celery-kos:$DOCKER_TAG" .

# Push if the push flag is set and not cross compiling
if [[ ( -n "${PUSH+x}" && -z "${CROSS_COMPILE+x}" ) ]]; then
    docker push "kenban/celery-kos:$DOCKER_TAG"
    docker push "kenban/celery-kos:latest"
fi
